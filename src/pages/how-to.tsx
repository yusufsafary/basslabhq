    ...
    <div className="w-full">
      ...
      <section className="py-20 bg-[#07111f]" id="steps">
        <div className="max-w-5xl mx-auto px-6 md:px-10">
          <p className="text-[10px] font-mono tracking-[0.28em] text-teal-400/70 uppercase mb-12 text-center">STEP BY STEP</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {steps.map((s,i) => (
              <motion.div key={i}
                initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }}
                viewport={{ once:true }} transition={{ delay:i*0.07 }}
                className="border border-white/7 bg-white/[0.025] p-6 group hover:border-amber-500/25 transition-all duration-300 relative">
      ...
      <section className="py-20 bg-[#060e1a] border-t border-white/5" id="species">
        <div className="max-w-6xl mx-auto px-6 md:px-10">
          <p className="text-[10px] font-mono tracking-[0.28em] text-amber-400/70 uppercase mb-12 text-center">SPECIES GUIDE</p>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            {/* Species list */}
            <div className="space-y-2">
              {species.map((s,i) => (
                <button key={i} onClick={() => setActiveSpecies(i)}
            ...
            <div className="lg:col-span-2 border border-white/7 bg-white/[0.025] p-7 relative overflow-hidden">
              ...
              <motion.div key={activeSpecies} initial={{ opacity:0, x:10 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.25 }}>
                ...
                <div className="mt-6">
                  <Link href="/generator">
                    <button className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-bold font-mono text-xs tracking-widest transition-colors">
                      GENERATE LOADOUT →
                    </button>
                  </Link>
                </div>
      ...
      <section className="py-20 bg-[#07111f] border-t border-white/5" id="tackle">
        <div className="max-w-3xl mx-auto px-6 md:px-10">
          <p className="text-[10px] font-mono tracking-[0.28em] text-teal-400/70 uppercase mb-12 text-center">TACKLE GLOSSARY</p>
          <div className="space-y-2">
            {tackleFAQ.map((item,i) => (
              <div key={i} className="border border-white/7 overflow-hidden">
                <button onClick={() => setOpenFAQ(openFAQ===i?null:i)}
                  ...
                  <span className={`text-amber-400 font-mono text-lg transition-transform duration-200 ${openFAQ===i?'rotate-45':''}`}>+</span>
                </button>
      ...
      <section className="py-20 border-t border-white/5 bg-[#060e1a]" id="tips">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <Link href="/generator">
            <button className="px-10 py-4 bg-amber-500 hover:bg-amber-400 text-black font-bold font-mono tracking-widest text-sm transition-colors shadow-xl shadow-amber-500/15">
              LAUNCH GENERATOR →
            </button>
          </Link>
        </div>
